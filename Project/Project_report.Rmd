...
sumdat$ariskstd<-with(sumdat,(arisk-mean(arisk))/arisk)
```
...
demlow2<-getData(name='SRTM',download=TRUE,path="C:/Classes/AppliedBiostat/Project",lon=35.98,lat=-.08)
#plot(dem)
...
lowel2<-readGDAL("srtm_44_13.tif")
...
lowel2<-crop(demlow2,lowext2,"LowSiteData2",overwrite=TRUE)
# image(lowel)
...
mriskLow<-focal(lowtr,fmat,filename='smoothLowTWI',pad=T,padValue=median(lowt$band1),overwrite=T)
mriskHigh<-focal(hightr,fmat,filename='smoothHighTWI',pad=T,padValue=median(hight$band1),overwrite=T)
```
...
lowsite$mriskstd<-(lowsite$mrisk-mean(lowsite$mrisk,na.rm=T))/sd(lowsite$mrisk,na.rm=T)
highsite$mriskstd<-(highsite$mrisk-mean(highsite$mrisk,na.rm=T))/sd(highsite$mrisk,na.rm=T)
```

```{r plotMrisk,echo=FALSE}
...
```{r plotCombrisk,echo=FALSE}
ggplot(data=lowp,aes(x=x,y=y))+geom_raster(aes(fill=layer))+scale_fill_gradient2("Elevation",low = "brown", mid = "yellow", high = "green",  midpoint = 1200, space = "rgb", na.value = "grey50", guide = "colourbar")+ geom_point(aes(x=lon, y=lat,color=comrsk),data=lowsite[-565,],alpha=0.9)+scale_colour_gradient("Combined Risk",low="blue",high="red")

ggplot(data=highp,aes(x=x,y=y))+geom_raster(aes(fill=layer))+scale_fill_gradient2("Elevation",low = "brown", mid = "yellow", high = "green",  midpoint = 1714, space = "rgb", na.value = "grey50", guide = "colourbar")+ geom_point(aes(x=lon, y=lat,color=comrsk),data=highsite,alpha=0.8)+scale_colour_gradient("Combined Risk",low="blue",high="red")
```

```{r checkLinearityAssumption,echo=FALSE,results='hold'}
rcspline.plot(lowsite$comrsk[-565],lowsite[-565,]$net,model="logistic",xlab="Combined Risk",ylab="Log Odds Net Use")#evidence of non-linearity (u-shape)
rcspline.plot(highsite$comrsk,highsite$net,model="logistic",xlab="Combined Risk")#evidence of non-linearity (increasing then leveling off)
rcspline.plot(lowsite$comrsk[-565],lowsite[-565,]$hspray,model="logistic")
rcspline.plot(highsite$comrsk,highsite$hspray,model="logistic")

rcspline.plot(lowsite$arisk[-565],lowsite[-565,]$net,model="logistic")
rcspline.plot(highsite$arisk,highsite$net,model="logistic")
rcspline.plot(lowsite$arisk[-565],lowsite[-565,]$hspray,model="logistic")
rcspline.plot(highsite$arisk,highsite$hspray,model="logistic")

rcspline.plot(lowsite$mrisk[-565],lowsite[-565,]$net,model="logistic")
...
rcspline.plot(lowsite$mrisk[-565],lowsite[-565,]$hspray,model="logistic")
...
m1ls<-glm(hspray~comrsk,data=lowsite[-565,],family="binomial")
...
m1ln<-glm(net~comrsk,data=lowsite[-565,],family="binomial")
...
m1mln<-glm(net~comrsk,data=highsite,family="binomial")
...
Site<-c("High","","Low","")
outmat<-round(matrix(c(m1ln.est,m1ls.est,m1hn.est,m1hs.est),4,3),2)
colnames(outmat)<-c("OR","Lower 95% CI","Upper 95% CI")
obj1out<-cbind(Site,Treatment,outmat)
```

Table 2. Odds of receiving a treatment as a function of combined risk.
...
m2als<-glm(hspray~arisk,data=lowsite[-565,],family="binomial")
...
m2aln<-glm(net~arisk,data=lowsite[-565,],family="binomial")
...
m2mls<-glm(hspray~mrisk,data=lowsite[-565,],family="binomial")
...
m2mln<-glm(net~mrisk,data=lowsite[-565,],family="binomial")
...
sepmat<-round(matrix(c(m2aln.est,m2mln.est,m2als.est,m2mls.est,m2ahn.est,m2mhn.est,m2ahs.est,m2mhs.est),4,6,byrow=T),2)
...
Table 3.  Odds of treatment from risk of either mosquito exposure or malaria risk.
