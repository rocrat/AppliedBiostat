...
run;
/*Estimates are coded to subtract the control from treatment*/
...
/*Perform MI with vars correlated >.15 and with treatment indicator*/
...
proc mianalyze parms = ests4;
	modeleffects control treatment difference;
run;
*Create sas tables to summarize missingness;
data lg;
set p.lung;
...
if qol_comp = 7 or qol_comp = 8 then qol_simp = "Missing from Illness";
if qol_comp = 3 or qol_comp = 5 then qol_simp="Missing from Staff";
if qol_comp = . then qol_simp= "Unknown";
qol_death = "Missing from Death";
if qol_simp ne "Missing from Death" then qol_death = "Missing from Other";
run;


*Complex summary of missingness;
...
run;
*Just death vs other cause for missingness;
proc tabulate data=lg;
label funo = "Time Point";
format funo funo_fmt.;
class funo qol_death exptx;
table  exptx*funo, qol_death*(N);
run;
